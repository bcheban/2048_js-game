...
function render(prevState = null) {
    ...
    cells.forEach((cell, index)=>{
        ...
        if (prevValue === 0 && value) cell.classList.add("new");
        if (value && value === prevValue * 2) cell.classList.add("merge");
        setTimeout(()=>{
...
function updateMessage() {
    ...
    if (status === "idle") document.querySelector(".message-start").classList.remove("hidden");
    if (status === "win") document.querySelector(".message-win").classList.remove("hidden");
    if (status === "lose") document.querySelector(".message-lose").classList.remove("hidden");
    if (status !== "idle") {
...
/* START / RESTART */ button.addEventListener("click", ()=>{
...
class Game {
    ...
    transpose(matrix) {
        ...
        for(let col = 0; col < 4; col++){
            result[col] = [];
            for(let row = 0; row < 4; row++)result[col][row] = matrix[row][col];
        }
        return result;
    }
    moveUp() {
